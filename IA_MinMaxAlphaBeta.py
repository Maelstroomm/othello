...
def MinMaxAlphaBeta(grille:np.array, evalMax: bool, joueur,score :list, profondeurMax:int, profondeur:int=0, indicesFils:list=[], coupsPossiblesPrecedant:list=[],alpha=float('-inf'),beta=float('inf')):
    """
    ...
    if profondeur>=profondeurMax :
        ...
        """
        if evalMax and joueur == NOIR:
            #print("evaluation : ",-e.evaluation2(grille, score,joueur))
            return (e.evaluation2(grille, score,joueur),None)
        elif evalMax and joueur == BLANC:
            #print("evaluation : ",e.evaluation2(grille, score,joueur))
            return ( -e.evaluation2(grille, score,joueur),None)
        elif not evalMax and joueur == NOIR:
            #print("evaluation : ",e.evaluation2(grille, score,joueur))
            return ( -e.evaluation2(grille, score,joueur),None)
        else :
            #print("evaluation : ",-e.evaluation2(grille, score,joueur))
            return (e.evaluation2(grille, score,joueur),None)
        #gerer les cas ou le joueur doit passer et ou la partie se finit avant d'atteindre la profondeur max
        """
        if joueur == NOIR :
        ...
        #cas recursif  
            ...
            if evalMax:
                ...
                for i in range(len(plateauxFils)):
                    result=MinMaxAlphaBeta(plateauxFils[i], not evalMax, joueur, scoresFils[i], profondeurMax,profondeur+1,coupsPossibles,alpha,beta)
                    gain=min(gain,result[0])
                    if alpha >= gain : # coupure alpha
                        return gain, indicesFils[i]
                    #print("coupure alpha")
                    beta = min(beta, gain)
            else : 
                gain=-inf
                for i in range(len(plateauxFils)):
                    result=MinMaxAlphaBeta(plateauxFils[i], not evalMax, joueur, scoresFils[i], profondeurMax,profondeur+1,coupsPossibles,alpha,beta)
                    gain=max(gain,result[0])
                    if gain >= beta: # coupure beta
                        return gain, indicesFils[i]
                    #print("coupure beta")
            ...
            #on rappelle MinMaxAlphaBeta() si tous les plateaux fils en changeant evalMax (car on change de joueur)                
            for i in range(len(plateauxFils)):
                #print(result)
                if result != None:
                    gainsDesFils.append(result[0])
                
                else:
                    gainsDesFils.append(-inf)
    ...
    while VarFinPartie == False :
        ...
        if coupsPossibles==[]:#si le joueur doit passer
            ...
            if dicoJoueurs[joueur]==HUMAIN:
                ...
                profondeurMaxi=4
                coordCoup=MinMaxAlphaBeta(grille,True, joueur, score,profondeurMaxi)[1]
            print(coordCoup)
